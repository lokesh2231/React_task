...
app.patch("/tasks/:id", async (req, res) => {
  ...
  try {
    const updatedTask = await Task.findByIdAndUpdate(req.params.id, { status }, { new: true }); // Update task and return the updated task
    if (!updatedTask) return res.status(404).json({ message: "Task not found." });
    res.json(updatedTask); // Respond with the updated task
  } catch (err) {
    console.error("Error updating task status:", err);
    res.status(500).json({ message: "Server error. Could not update task status." });
  }
});
